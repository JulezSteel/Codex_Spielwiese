...
export async function POST(request: NextRequest) {
  try {
    ...
    if (config.provider === "gemini" && process.env.GEMINI_API_KEY) {
      const genUrl = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent";
      const genRes = await fetch(`${genUrl}?key=${process.env.GEMINI_API_KEY}`, {
        method: "POST",
